...
const STATUS_LABEL: Record<
  PhotoSyncProgressState['stages'][PhotoSyncProgressStage]['status'],
  string
> = {
  pending: '等待中',
  running: '进行中',
...
export const PhotoSyncProgressPanel = ({
  progress,
}: PhotoSyncProgressPanelProps) => {
    ...
    <div className="bg-background-tertiary relative overflow-hidden rounded-lg p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-sm text-text-tertiary">
          <span className="relative inline-flex h-3 w-3">
            ...
            <span
              className={`relative inline-flex h-3 w-3 rounded-full ${isErrored ? 'bg-rose-500' : 'bg-accent'}`}
            />
          </span>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {stageItems.map((item) => (
          <div
            key={item.stage}
            className="border-border/20 bg-fill/10 relative overflow-hidden rounded-lg border p-4"
          >
            <BorderOverlay />
            ...
            <div className="mt-3 h-1.5 rounded-full bg-fill/30">
              <m.div
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        {summaryItems.map((item) => (
          <div
            key={item.label}
            className="bg-background-secondary/60 border-border/20 rounded-lg border p-4"
          >
        ...
        <div className="border-border/20 bg-fill/10 mt-6 overflow-hidden rounded-lg border p-4">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-sm">
            ...
            <code className="text-text-secondary text-xs">
              {lastAction.action.storageKey}
            </code>
            <span className="text-text-tertiary text-xs">
              {STAGE_CONFIG[lastAction.stage].label}
            </span>
          </div>
          <p className="text-text-tertiary mt-2 text-xs">
            {lastAction.total > 0
              ? `进度：${lastAction.index} / ${lastAction.total}`
              : '无需进一步处理'}
          </p>
        </div>
      ) : null}
      ...
      {progress.error ? (
        <div className="border-rose-500/40 bg-rose-500/10 mt-6 rounded-lg border p-4 text-sm text-rose-200">
          {progress.error}
        </div>
      ) : null}
    </div>
